...
function Model(databaseHandle, options) {
	...
	function createRequestHandler(model) {
		...
		return function onRequest(request, response) {
			...
			if (path === '/') { // index
			...
			} else if (path.match(/^\/[^\/]+$/)) { // findOneByID
				if (request.method === 'GET' && !apiOptions.byID) {
				...
				} else if (request.method === 'PUT' && !apiOptions.save) {
					respondError(403, 'Save Is Not Allowed');
				} else if (request.method === 'PUT') {
					request.on('readable', function() {
						var body = JSON.stringify(request.read());
					});
				}
			} else if (
...
Model.prototype = {
	...
	findAll: function(callback) {
		...
		this._db.list(function(error, results) {
			if (error) {
				callback(error, null);
			} else {
				var instances = [];

 ...
 * @param {boolean} options.restapi.by_id enable `GET /{id} requests`
 */
		...
		return function onRequest(request, response) {
			...
			if (path === '/') { // index
				if (!apiOptions.index) {
					...
					model._db.list(function(error, body) {
						if (error) {
			...
			} else if (path.match(/^\/[^\/]+$/)) { // by_id
				if (!apiOptions.byID) {
					return respondError(403, 'ByID Queries Not Allowed');
				} else {
					model._db.get(path.replace(/^\//, ''), function(error, body) {
